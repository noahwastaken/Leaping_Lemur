/*
 *  drivers/cpufreq/cpufreq_lionheartX.c
 *
 * Modified and Tweaked by: Lens-F
 * 
 * Based on the lionheartX Governor By:
 *
...
static unsigned int skip_lionheartX = 0;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_lionheartX
static
#endif
struct cpufreq_governor cpufreq_gov_lionheartX = {
       .name                   = "lionheartX",
...
extern int has_boost_cpu_func;
	...
	lionheartX_early_suspend_handler.suspend = lionheartX_early_suspend;
	lionheartX_early_suspend_handler.resume = lionheartX_late_resume;
	lionheartX_early_suspend_handler.level = EARLY_SUSPEND_LEVEL_DISABLE_FB + 100;
#endif
...
#ifdef CONFIG_CPU_FREQ_GOV_lionheartX_MULTI_PHASE
#define TABLE_SIZE			5
...
bool is_lionheartX_locked(void)
{
...
EXPORT_SYMBOL(is_lionheartX_locked);
...
static int lionheartX_powersave_bias_setspeed(struct cpufreq_policy *policy,
...
static void lionheartX_powersave_bias_init_cpu(int cpu)
{
...
static void lionheartX_powersave_bias_init(void)
{
	int i;
	for_each_online_cpu(i) {
		lionheartX_powersave_bias_init_cpu(i);
	}
}

void lionheartX_boost_cpu(int boost)
{
	int cpu;
...
EXPORT_SYMBOL(lionheartX_boost_cpu);
...
skip_this_cpu:
		...
		lionheartX_powersave_bias_init();
	} else {
		for_each_online_cpu(cpu) {
			if (lock_policy_rwsem_write(cpu) < 0)
				continue;
			...
			if (dbs_info->cur_policy) {
				...
				lionheartX_powersave_bias_setspeed(
					dbs_info->cur_policy,
					NULL,
					input);
				   ...
				   unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		this_dbs_info->rate_mult = 1;
		lionheartX_powersave_bias_init_cpu(cpu);
		if (dbs_enable == 1) {
		...
		if (!lionheartX_powersave_bias_setspeed(
...
#ifndef CONFIG_CPU_FREQ_GOV_lionheartX_MULTI_PHASE
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_lionheartX);
}
...
static void __exit cpufreq_gov_dbs_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_lionheartX);
	for_each_possible_cpu(i) {
		struct cpu_dbs_info_s *this_dbs_info =
...
MODULE_AUTHOR("Tal Noah <tnoah12@gmail.com>");
MODULE_AUTHOR("Venkatesh Pallipadi <venkatesh.pallipadi@intel.com>");
MODULE_AUTHOR("Alexey Starikovskiy <alexey.y.starikovskiy@intel.com>");
MODULE_DESCRIPTION("'cpufreq_lionheartX' - A dynamic yet agile cpufreq governor for based off ondemand, conservative, and lionheart governors");
MODULE_LICENSE("GPL");

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_lionheartX
